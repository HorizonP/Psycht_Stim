%real_whiteNoise: generate whitenoise by certain frequency and interval. This version calculate noise during each frame, protect from out of memory error for long time mapping. 
...
while (~KbCheck)&&(i<=times)
    im=MaxIntensity/(level-1)*randi([0,level-1],pixNum);
    imArr=[imArr im];
